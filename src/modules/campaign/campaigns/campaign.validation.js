...
const baseCampaignSchema = {
  title: Joi.string().trim().min(3).max(255).optional().allow(null, '').messages({
  ...
  goalAmount: Joi.number().positive().precision(2).optional().allow(null).messages({
  ...
  categoryIds: Joi.array().items(Joi.string().uuid()).optional().allow(null).messages({
...
export const updateCampaignSchema = Joi.object({
  ...
  status: Joi.string()
    ...
    .messages({
      "any.only":
        "Status must be one of: draft, pendingApproval, active, successful, closed, cancelled, rejected.",
    }),
});
