...
export const updateUserProfile = async (userId, profileData) => {
  try {
    ...
    logger.error('Failed to update user profile in service', { error: error.message, userId });
 ...
 */
export const updateProfileImage = async (userId, profilePictureFile, coverPictureFile) => {
  // Only for individual users for now
  const { user, profile } = await getUserWithProfileById(userId);
  if (!user || !profile) throw new NotFoundError('User/profile not found');
  if (user.user_type !== 'individual_user') throw new DatabaseError('Only individual users can update profile images');
  ...
  await transaction(async (client) => {
    if (profilePictureMediaRecord) {
      ...
      await userRepository.updateProfileMediaId(userId, 'profile_picture_media_id', profilePictureMediaId, client);
      logger.info('Profile picture media record created and profile updated in database', { 
        userId, 
        mediaId: profilePictureMediaId 
      });
    }
    if (coverPictureMediaRecord) {
      ...
      await userRepository.updateProfileMediaId(userId, 'cover_picture_media_id', coverPictureMediaId, client);
      logger.info('Cover picture media record created and profile updated in database', { 
        userId, 
        mediaId: coverPictureMediaId 
      });
    }
  ...
  const { user: updatedUser, profile: updatedProfile } = await getUserWithProfileById(userId);
